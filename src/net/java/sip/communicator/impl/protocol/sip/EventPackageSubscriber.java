    ...
    extends EventPackageSupport
{
    ...
    /**
     * The number of seconds to wait after a failed subscription request
     * before retrying
     */
    protected static final int retryMargin = 5 * 60;
    ...
    public boolean processRequest(RequestEvent requestEvent)
    {
        ...
        if (SubscriptionStateHeader.TERMINATED.equalsIgnoreCase(sstate))
        {
            ...
            if (subscription != null)
            {
                ...
                subscription.processTerminatedRequest(requestEvent,
                ...
                && (statusCode < Response.MULTIPLE_CHOICES))
        {
                 ...
                 && (statusCode < Response.BAD_REQUEST))
         {
             logger.info(
                     ...
                     + ". Retrying subscription in " + retryMargin + "s.");

             // Retry the subscription later
             retrySubscribe(subscription, retryMargin);
         }
         else if(statusCode >= Response.BAD_REQUEST)
         {
             ...
             {
                 boolean responseHandled = handleIntervalTooBrief(subscription,
                                                                  request,
                                                                  response);
             ...
             // 481 - dialog does not exist
             else if (statusCode == Response.CALL_OR_TRANSACTION_DOES_NOT_EXIST)
             {
                 logger.debug("Transaction does not exist from network.");

                 removeSubscription(callId, subscription);
                 subscription.processFailureResponse(responseEvent, statusCode);

                 // Clear the dialog so that we create a new one. Note that this
                 // is safe since the subscription is originally created with a
                 // null dialog
                 subscription.setDialog(null);

                 // Retry the subscription immediately. By creating a new dialog
                 // we ensure that we won't hit this again, thus can trying a
                 // faster recovery than normal
                 retrySubscribe(subscription, 1);
             }
    ...
    private boolean handleIntervalTooBrief(Subscription subscription,
                                           Request request,
                                           Response response)
    {
        ...
        throws OperationFailedException
    {
        ...
        {
            // Failed to create the subscription, set up a timer to try again
            // in 1 minute.
            retrySubscribe(subscription, 60);

            ProtocolProviderServiceSipImpl.throwOperationFailedException(
                "Failed to create the subscription. Setting retry timer.",
                OperationFailedException.INTERNAL_ERROR, ex, logger);
        }
        catch (IllegalArgumentException ex)
        {
            // This exception is from ProtocolProviderServiceSipImpl.getIntendedDestination() not finding any address.
            // It could be due to DNS network problems meaning we fail to resolve the domain, so retry.
            retrySubscribe(subscription, 60);

            ProtocolProviderServiceSipImpl.throwOperationFailedException(
                "Failed to create the subscription. Setting retry timer.",
                OperationFailedException.INTERNAL_SERVER_ERROR, ex, logger);
        }
    ...
    /**
     * Sets up a timer task to retry the subscription after a number of
     * seconds.
     *
     * @param subscription
     *            a <tt>Subscription</tt> which specifies the properties of
     *            the SUBSCRIBE request to be created and sent, to be added to
     *            the list of subscriptions managed by this instance
     * @param retryMargin the number of seconds to wait before retrying
     **/
    protected void retrySubscribe(Subscription subscription, int retryMargin)
    {
        SubscriptionRefreshTask refreshTask
                                = new SubscriptionRefreshTask(subscription);
        ...
        try
        {
            logger.debug("Scheduling the subscription refresh task");
            timer.schedule(refreshTask, retryMargin * 1000);
        }
        catch (IllegalStateException ex)
        {
            // From the TimerTask JavaDoc:
            // IllegalStateException - if task was already scheduled or
            // cancelled, timer was cancelled, or timer thread terminated.
            //
            // We have only just created the task so it must mean the timer was
            // cancelled or the thread was terminated. Either way, let's
            // recreate the timer
            logger.warn("Scheduling of subscription refresh task failed - " +
                        "recreating the timer and trying again");
               ...
               OperationFailedException
    {
        ...
        if (dialog != null)
        {
            ...
            {
                // We're no longer subscribed to this contact - this ensure that the response of
                // this request will be handled as an unsubscription response.  Note we call this
                // even if we fail to send the unSUBSCRIBE request, because nothing will re-attempt
                // to send the request if it fails, and we need to remove our subscription for
                // necessary clean-up to occur.
                removeSubscription(callId, subscription);
            }
        ...
        extends TimerTask
    {
        ...
        public void run()
        {
            ...
            if (dialog == null)
            {
                logger.info("null dialog associated with " + subscription + ", trying to create new request");

                // Timer has popped for retrying a failed subscribe request.
                // Generate the new subscribe request now.
                try
                {
                    subscribe(subscription);
                }
                ...
                {
                    logger.error("Failed to create and send subcription", e);
                }
                return;
            }
