    ...
    implements MessageFactoryExt
{
     ...
     * messages).
     */
    private final MessageFactoryExt wrappedFactory;
    ...
    public SipMessageFactory(ProtocolProviderServiceSipImpl service,
            MessageFactoryExt wrappedFactory)
    {
        if (service == null)
    ...
    public Request createRequest(URI requestURI, String method, CallIdHeader
            callId, CSeqHeader cSeq, FromHeader from, ToHeader to, List<ViaHeader> via,
    ...
    public Response createResponse(int statusCode, CallIdHeader callId,
            CSeqHeader cSeq, FromHeader from, ToHeader to, List<ViaHeader> via,
                MaxForwardsHeader maxForwards, ContentTypeHeader contentType,
                Object content)
        throws ParseException
    {
    ...
    public MultipartMimeContent createMultipartMimeContent(ContentTypeHeader multipartMimeContentTypeHeader,
                                                           ...
                                                           String[] contentBody)
    {
        return this.wrappedFactory.createMultipartMimeContent(
    ...
    /**
     * Create an empty multipart mime body.  This will have the correct
     * Content-Type header set for it, but will contain no body parts.  The
     * caller is responsible for adding the required body parts, prior to use.
     *
     * @return An empty multipart mime body
     */
    public MultipartMimeContent createMultipartMimeContent()
    {
        MultipartMimeContent multipart = null;
        try
        {
            ...
            String boundaryString = "boundaryString-" + this.hashCode();
            contentTypeHeader.setParameter(MultipartMimeContentImpl.BOUNDARY,
                                           boundaryString);
            multipart = this.wrappedFactory.createMultipartMimeContent(
                    contentTypeHeader,
                    new String[0],
                    new String[0],
                    new String[0]);
        ...
        {
            logger.error("Failed to create empty multipart content");
        }
     ...
     */
    @VisibleForTesting
    Message attachScSpecifics(Message message)
    {
    ...
    public void preAuthenticateRequest( Request request )
    {
        ...
        SipSecurityManager securityManager = protocolProvider
                .getSipSecurityManager();
        AuthorizationHeader authorization = securityManager
                .getCachedAuthorizationHeader(callid);

        if (authorization != null)
            request.setHeader(authorization);

        // If we're using 3GPP mediasec headers and this request is not an ACK,
        // PRACK or CANCEL (ACKs do not use this method due to checks further up
        // the stack), try to add a Security-Verify header. See excluded header
        // types in RFC 3329.
        String method = request.getMethod();
        if (protocolProvider.is3GPPMediaSecurityEnabled() &&
            !Request.CANCEL.equals(method) &&
            !Request.PRACK.equals(method))
        {
            logger.debug("Asked to add Security-Verify header for " +
                         "mediasec in " + method + " request");
            protocolProvider.getSecurityHeaderFactory().addSecurityVerifyHeader(
                    request, securityManager.getSecurityServerCache());
        }
    }
               ...
               OperationFailedException
    {
        ...
        //Via
        List<ViaHeader> viaHeaders = protocolProvider
        ...
        // If we're using 3GPP media security, add Security-Client, Require and
        // Proxy-Require headers.
        if (protocolProvider.is3GPPMediaSecurityEnabled())
        {
            logger.debug("Adding 3GPP mediasec headers to REGISTER");
            protocolProvider.getSecurityHeaderFactory()
                    .addMediaSecurityHeaders(request);
        }
    ...
    /**
     * Creates a CANCEL Request for the specified INVITE transaction
     *
     * @param transaction the transaction to CANCEL.
     * @return a <tt>Request</tt> built to cancel the INVITE transaction.
     * @throws SipException if we fail constructing the request for
     * some reason.
     */
    public Request createCancelRequest(ClientTransaction transaction)
        throws SipException
    {
        Request cancelRequest = transaction.createCancel();

        attachScSpecifics(cancelRequest);
