    ...
    extends AbstractContactGroupJabberImpl
{
    ...
    ContactGroupJabberImpl(
                        ...
                        boolean isResolved)
    {
        // rosterGroup is non-null if and only if the contact group we are
        // creating is not the volatile group.
        if(rosterGroup != null)
        {
            this.id = rosterGroup.getName();

            // Log an error for non-volatile contact groups.
            // We shouldn't ever see any as they're unsupported and the
            // ServerStoredContactList shouldn't create them.
    ...
    ContactGroupJabberImpl(
                        ...
                        ServerStoredContactListJabberImpl ssclCallback)
    {
        // Log an error for unresolved contact groups.
        // We shouldn't ever see any as they're unsupported and the
        // ServerStoredContactList shouldn't create them.
        sLog.error("Unexpected contact group created: " + id);
        this.tempId = id;
    ...
    public Iterator<Contact> contacts()
    {
        ...
        Map<String, Contact> contacts = new Hashtable<>(buddies);
    ...
    public boolean equals(Object obj)
    {
        ...
        String groupName = ((ContactGroup)obj).getGroupName();

        if(groupName == null || !groupName.equals(getGroupName()))
            return false;
    ...
    ContactJabberImpl findContact(Jid id)
    {
        if(id == null)
            return null;
        return (ContactJabberImpl)buddies.get(id.toString().toLowerCase());
    }
