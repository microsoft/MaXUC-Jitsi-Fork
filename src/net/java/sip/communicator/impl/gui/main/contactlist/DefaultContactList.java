 ...
 * @author Adam Netocny
 */
public class DefaultContactList<T>
    extends JList<T>
    implements Skinnable
{
     ...
     */
    private MouseEvent mCachedMouseEvent;

    /**
     * List cell renderer.
     */
    ContactListCellRenderer<T> mRenderer = new ContactListCellRenderer<>();
    ...
    public JToolTip createToolTip()
    {
        ...
        int index = locationToIndex(currentMouseLocation);
        ...
        if (element instanceof MetaContact)
        {
            ...
            BufferedImageFuture avatarImage = metaContact.getAvatar();

            if (avatarImage != null)
                tip.setImage(new ImageIcon(avatarImage.resolve()));
        ...
        else if (element instanceof ChatContact<?>)
        {
            ...
            ImageIconFuture avatarImage = chatContact.getAvatar();

            if (avatarImage != null)
                tip.setImage(avatarImage.resolve());
    ...
    protected void fireSelectionValueChanged(int firstIndex, int lastIndex,
                                            boolean isAdjusting)
    {
        if (mCachedMouseEvent != null)
        {
            super.processMouseEvent(new MouseEvent(
                (Component) mCachedMouseEvent.getSource(),
                mCachedMouseEvent.getID(),
                mCachedMouseEvent.getWhen(),
                mCachedMouseEvent.getModifiers(),
                mCachedMouseEvent.getX(),
                mCachedMouseEvent.getY(),
                mCachedMouseEvent.getClickCount(),
                mCachedMouseEvent.isPopupTrigger()));

            mCachedMouseEvent = null;
        }
