    ...
    implements Runnable
{
     ...
     */
    private final Map<SystemActivityEvent, Integer> eventsToDispatch
            = new LinkedHashMap<>();
        ...
        SystemActivityChangeListener listener)
    {
        synchronized(listeners)
        {
            if(!listeners.contains(listener))
            {
                ...
                {
                    dispatcherThread = new Thread(this, "SystemActivityEventDispatcher");
    ...
    protected void fireSystemActivityEvent(SystemActivityEvent evt, int wait)
    {
        synchronized(eventsToDispatch)
        {
            ...
            if(dispatcherThread == null && listeners.size() > 0)
            {
                ...
                dispatcherThread.setName("SystemActivityEventDispatcher");
                dispatcherThread.start();
            }
        }
    }
