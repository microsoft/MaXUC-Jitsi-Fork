...
public class Messenger
{
    ...
    static final int MISTATUS_BUSY             = 0x000A;
    static final int MISTATUS_MEETING          = 0x0052;
    ...
    static final int MISTATUS_DO_NOT_DISTURB   = 0x0072;
     ...
     * order position.
     */
    private static final int[] MPHONE_TYPE_SORT_ORDER = new int[]
        {
            MPHONE_TYPE_WORK,
            MPHONE_TYPE_HOME,
            MPHONE_TYPE_MOBILE,
     ...
     * to invoke {@link UIService#createCall(Reformatting, String[])} with a phone number
    ...
    /**
     * The <tt>RegistrationStateChangeListener</tt> which listens to the
     * <tt>BundleContext</tt> in which the <tt>msofficecomm</tt> bundle has
     * been started for registration state changes.
     */
    private static final RegistrationStateChangeListener registrationListener = new RegistrationStateChangeListener()
        {
            public void registrationStateChanged(RegistrationStateChangeEvent event)
            {
                Messenger.registrationStateChanged(event);
            }
        };
     ...
     */
    @VisibleForTesting
    static List<Contact> findContactsBySigninName(
            ...
            String signinName)
    {
        ...
        // First check if there's any metacontact that has
        for (Iterator<MetaContact> metaContactIt = metaContactListService.findAllMetaContactsForProvider(pps); metaContactIt.hasNext();)
        {
            ...
            for (Iterator<Contact> contactIt = metaContact.getContacts(); contactIt.hasNext();)
            {
                ...
                {
                    /*
                     * Prefer matches by Contact address over an email address.
                     */
        ...
        // If we didn't find any contacts matching the sign-in address as their account address, look for metacontacts
        // with the sign-in address as an email address, and see if any of those have corresponding contacts for the pps.
        if (contacts.isEmpty())
        {
            for (MetaContact metaContact : metaContactListService.findMetaContactByEmail(signinName))
            {
                logger.debug(logHasher(metaContact.getDisplayName()) + " matches email address");
                Iterator<Contact> it = metaContact.getContactsForProvider(pps);
            ...
            int limit)
    {
        ...
        Set<PhoneNumberDetail> phoneNumbers = new TreeSet<>(new Comparator<PhoneNumberDetail>()
                {
    ...
    static int getStatus(MessengerContact messengerContact)
    {
        ...
        logger.debug("Got getStatus for " + logHasher(signinName));
            ...
            if (self == null)
            {
                ...
                for (Self aSelf : selves.values())
                {
                    ...
                    if (aPresenceStatus != null)
                    {
                        ...
                        if (presenceStatus.toInt() >= PresenceStatus.MAX_STATUS_VALUE)
                        {
                            break;
                        }
                    }
        ...
        logger.debug(logHasher(signinName) + " has status " + ProtocolPresenceStatus.toMISTATUS(presenceStatus));
    ...
    private static native void onContactStatusChange(String signinName, int status);
    ...
    private static void serviceChanged(ServiceEvent event)
    {
        ...
        switch (event.getType())
        {
            case ServiceEvent.REGISTERED:
                logger.debug("Service registered " + pps);
                pps.addRegistrationStateChangeListener(registrationListener);

                // Check whether the provider registered while we were setting
                // up the listener.
                if (pps.getRegistrationState() == RegistrationState.REGISTERED)
                {
                    logger.debug("Provider already registered " + pps);
                    registrationListener.registrationStateChanged(
                        new RegistrationStateChangeEvent(
                            pps, null, RegistrationState.REGISTERED, -1, null));
                }
                break;

            case ServiceEvent.UNREGISTERING:
                logger.debug("Service unregistering " + pps);
                pps.removeRegistrationStateChangeListener(registrationListener);
                registrationListener.registrationStateChanged(
                    new RegistrationStateChangeEvent(
                        pps, null, RegistrationState.UNREGISTERING, -1, null));
                break;
        }
    }

    /**
     * Adds and removes instances of ProtcolProviderService from the list of
     * those who support integration with Microsoft Outlook when their accounts
     * become registered or unregistered.
     *
     * @param event The <tt>RegistrationStateChangeEvent</tt> that indicates
     * whether the account has registered or is unregistering.
     */
    private static void registrationStateChanged(RegistrationStateChangeEvent event)
    {
        RegistrationState newState = event.getNewState();

        // We only care about certain state changes
        if (!(newState.equals(RegistrationState.REGISTERED) ||
             newState.equals(RegistrationState.UNREGISTERED) ||
             newState.equals(RegistrationState.UNREGISTERING)))
        {
            return;
        }

        ProtocolProviderService pps = (ProtocolProviderService)event.getSource();
        ...
        if (presenceOpSet != null)
        {
            ...
            if (signinName != null)
            {
                String loggableSigninName = logHasher(signinName);
                if (RegistrationState.REGISTERED.equals(newState))
                {
                    logger.info(loggableSigninName + " registered");
                    addSelf(signinName, pps, presenceOpSet);
                }
                else
                {
                    logger.info(loggableSigninName + " unregistering");
    ...
    public void startConversation(final int conversationType, String[] participants, String conversationData)
    {
                    ...
                    try
                    {
                        ...
                        if ("TelURIs".equalsIgnoreCase(documentElement.getTagName()))
                        {
                            ...
                            if (childNodes != null)
                            {
                                ...
                                List<String> phoneNumbers = new ArrayList<>(childNodeCount);
        ...
        boolean createCallByPhoneNumber = (cfg != null) && cfg.global().getBoolean(PNAME_CREATE_CALL_BY_PHONE_NUMBER, false);
                    ...
                    public void run()
                    {
                        ...
                        if (bundleContext != null)
                        {
                            ...
                            if (uiService != null)
                            {
                                ...
                                case CONVERSATION_TYPE_PHONE:
                                case CONVERSATION_TYPE_PSTN:
                                    sendAnalyticsEvent(AnalyticsEventType.OUTLOOK_CALL);
                                    uiService.createCall(Reformatting.NEEDED, contactArray);
                                    break;
                                case CONVERSATION_TYPE_IM:
                                    sendAnalyticsEvent(AnalyticsEventType.OUTLOOK_IM);

                                    // Should only send the WISPA Motion if there is
                                    // only one contact, since the current implementation
                                    // provided by <tt>UIServiceImpl</tt> supports a
                                    // single participant only.
                                    if (contactArray.length == 1)
                                    {
                                        WISPAService wispaService = ServiceUtils.getService(bundleContext,
                                                                                            WISPAService.class);
                                        if (wispaService != null)
                                        {
                                            List<Contact> contacts = findContactsBySigninName(contactArray[0],
                                                                                              opSetClass,
                                                                                              1);
                                            if (!contacts.isEmpty())
                                            {
                                                Contact contact = contacts.get(0);
                                                MetaContact metaContact = metaContactListService.findMetaContactByContact(
                                                        contact);
                                            ...
                                            {
                                                logger.error("No contact found for participant: " +
                                                             Hasher.logHasher(contactArray[0]));
                                            }
                                        }
                                        else
                                        {
                                            logger.error("Could not send WISPA Motion because" +
                                                         "WISPA service is unavailable.");
                                        }
                                    }
                                    break;
                                }
    ...
    private void sendAnalyticsEvent(AnalyticsEventType eventType)
    {
        AnalyticsService analyticsService = MsOfficeCommActivator.getAnalyticsService();

        if (analyticsService != null)
        {
            analyticsService.onEvent(eventType);
        }
    }
    ...
    private static class ProtocolPresenceStatus
    {
        ...
        {
            return toMISTATUS(presenceStatus);
        }
        ...
        {
            return (protocolPresenceStatus == null) ? MISTATUS_UNKNOWN : protocolPresenceStatus.toMISTATUS();
        }
        ...
        public static int toMISTATUS(PresenceStatus presenceStatus)
        {
            ...
            {
                if (i < PresenceStatus.ONLINE_THRESHOLD)
                {
                    mistatus = MISTATUS_OFFLINE;
                }
                else if (i <= PresenceStatus.DND_THRESHOLD)
                {
                    mistatus = MISTATUS_DO_NOT_DISTURB;
                }
                else if (i <= PresenceStatus.BUSY_THRESHOLD)
                {
                    mistatus = MISTATUS_BUSY;
                }
                else if (i <= PresenceStatus.ON_THE_PHONE_THRESHOLD)
                {
                    mistatus = MISTATUS_ON_THE_PHONE;
                }
                else if (i <= PresenceStatus.MEETING_THRESHOLD)
                {
                    mistatus = MISTATUS_MEETING;
                }
     ...
     */
    private static class Self implements ContactPresenceStatusListener, ProviderPresenceStatusListener
    {
                ...
                ContactPresenceStatusChangeEvent event)
        {
            ...
            if (signinName != null)
            {
                ...
                PresenceStatus newStatus = event.getNewStatus();

                int oldMISTATUS = ProtocolPresenceStatus.toMISTATUS(oldStatus);
                int newMISTATUS = ProtocolPresenceStatus.toMISTATUS(newStatus);

                if (oldMISTATUS != newMISTATUS)
                {
                    logger.debug("Updating " + logHasher(signinName) + " from " + oldMISTATUS + " to " + newMISTATUS);

                    // The onContactStatusChanged method notifies Outlook
                    // that a status has changed. However, it actually gets
                    // the updated status by calling get_PresenceProperties()
                    // (in native/windows/msofficecomm/MessengerContact.cxx),
                    // which then calls the Java method Messenger.getStatus().
                    Messenger.onContactStatusChange(signinName, newMISTATUS);
                }
            }
        }
                ...
                List<Contact> contacts)
        {
            for (Map.Entry<ProtocolProviderService, OperationSetPresence> e : ppss.entrySet())
            {
                ...
                OperationSetContactCapabilities contactCapabilitiesOpSet =
                    (opSetClass == null) ? null : pps.getOperationSet(OperationSetContactCapabilities.class);

                for (Contact contact : Messenger.findContactsBySigninName(pps, e.getValue(), signinName))
                {
                    if ((contactCapabilitiesOpSet == null) ||
                        (contactCapabilitiesOpSet.getOperationSet(contact, opSetClass) != null))
                    {
                ...
                Set<PhoneNumberDetail> phoneNumbers)
        {
            for (Map.Entry<ProtocolProviderService, OperationSetPresence> e : ppss.entrySet())
            {
                ...
                for (Contact contact :
                         Messenger.findContactsBySigninName(pps, e.getValue(), signinName))
                {
                    Iterator<PhoneNumberDetail> iter =
                        serverStoredContactInfoOpSet.getDetailsAndDescendants(contact, PhoneNumberDetail.class);
                    ...
                    try
                    {
                        ...
                        for (Contact contact : contacts)
                        {
                            ...
                            if (contactPresenceStatus != null)
                            {
                                if (presenceStatus == null)
                                {
                                    presenceStatus = new ProtocolPresenceStatus(contactPresenceStatus);
                                }
                                else if (presenceStatus.compareTo(contactPresenceStatus) < 0)
                                {
                                    presenceStatus.setPresenceStatus(contactPresenceStatus);
                                }
                                if (presenceStatus.toInt() >= PresenceStatus.MAX_STATUS_VALUE)
                                {
                                    break;
                        ...
                        if ((presenceStatus != null) &&
                            (presenceStatus.toInt() >= PresenceStatus.MAX_STATUS_VALUE))
                        {
                            break;
                        }
                    }
        ...
        public void providerStatusChanged(ProviderPresenceStatusChangeEvent event)
        {
            ...
            for (Map.Entry<ProtocolProviderService, OperationSetPresence> e : ppss.entrySet())
            {
                ...
                if (presenceOpSetStatus != null)
                {
                    ...
                    {
                        protocolPresenceStatus = new ProtocolPresenceStatus(presenceOpSetStatus);
                    }
                    else if (protocolPresenceStatus.compareTo(presenceOpSetStatus) < 0)
                    {
                        protocolPresenceStatus.setPresenceStatus(presenceOpSetStatus);
                    }
                    if (protocolPresenceStatus.toInt() >= PresenceStatus.MAX_STATUS_VALUE)
                    {
                        break;
                    }
                }
        ...
        private void setPresenceStatus(ProtocolPresenceStatus presenceStatus)
        {
            ...
            if (oldMISTATUS != newMISTATUS)
            {
                ...
                logger.debug("Updating user status (" + logHasher(signinName) + ") from " + oldMISTATUS + " to " + newMISTATUS);
