        ...
        implements ActionListener
{
    ...
    private JComboBox<SipPresenceTypeEnum> presenceTypeCombo =
            new JComboBox<>(SipPresenceTypeEnum.values());
    ...
    private String[] contactlistOptions = new String[]
            { "Local", "XCAP", "XiVO", "CommPortal" };

    private JComboBox<String> contactlistOptionsCombo = new JComboBox<>(
            contactlistOptions);
    ...
    public PresencePanel(SIPAccountRegistrationForm regform)
    {
        ...
        // Initialize the presence type combo box and label, and a panel to
        // contain them - a FlowLayout panel is an easy way of making the
        // combo box and label only take the space they need.
        JPanel presenceTypePanel =
            new TransparentPanel(new FlowLayout(FlowLayout.LEFT));
        presenceTypePanel.add(new JLabel(
            Resources.getString("plugin.sipaccregwizz.PRESENCE_TYPE")));
        presenceTypePanel.add(presenceTypeCombo);
        buttonsPresOpPanel.add(presenceTypePanel);
        presenceTypeCombo.addActionListener(this);
        setPresenceType(regform.getRegistration().getPresenceType());

        // Initilize the panel containing the "force p2p?" button
        JPanel simplePresencePanel =
            new TransparentPanel(new BorderLayout());
        simplePresencePanel.add(forceP2PPresOpButton, BorderLayout.WEST);
        simplePresencePanel.setBorder(
            BorderFactory.createEmptyBorder(0, 16, 0, 0));
        buttonsPresOpPanel.add(simplePresencePanel);
    ...
    {
        presenceTypeCombo.setEnabled(isEnabled);
    ...
    /**
     * Returns the SIP presence type (one of SIMPLE, DIALOG)
     *
     * @return The SIP presence type
     */
    SipPresenceTypeEnum getPresenceType()
    {
        return (SipPresenceTypeEnum)presenceTypeCombo.getSelectedItem();
    }

    /**
     * Sets the SIP presence type
     *
     * @param presenceType one of SIMPLE, DIALOG
     */
    void setPresenceType(SipPresenceTypeEnum presenceType)
    {
        presenceTypeCombo.setSelectedItem(presenceType);
    }
    ...
    /**
     * Indicates if CommPortal has to use its capabilities.
     *
     * @return <tt>true</tt> if CP has to use its capabilities,
     *         <tt>false</tt> - otherwise.
     */
    boolean isCpEnable()
    {
        Object o = contactlistOptionsCombo.getSelectedItem();
        if(o != null && o.equals(contactlistOptions[3]))
            return true;
     ...
     * Sets if has to use its capabilities.
     *
     * @param cpEnable if has to use its capabilities.
     */
    void setCpEnable(boolean cpEnable)
    {
        if(cpEnable)
        {
            contactlistOptionsCombo.setSelectedItem(contactlistOptions[3]);
            updateContactListPanel();
        }
    }
    ...
    {
        // Update the correct settings pane, based on what field has been
        // changed.
        Object source = actionEvent.getSource();
        if (contactlistOptionsCombo.equals(source))
        {
            updateContactListPanel();
        }
        else if (presenceTypeCombo.equals(source))
        {
            updateSimplePresencePanel();
        }
    }
    ...
    /**
     * Updates whether the force P2P button is enabled
     */
    private void updateSimplePresencePanel()
    {
        if (SipPresenceTypeEnum.DIALOG.equals(presenceTypeCombo.getSelectedItem()))
        {
            // SIP DIALOG presence is selected, so P2P is not available.
            forceP2PPresOpButton.setEnabled(false);
        }
        else
        {
            forceP2PPresOpButton.setEnabled(true);
        }
    }
