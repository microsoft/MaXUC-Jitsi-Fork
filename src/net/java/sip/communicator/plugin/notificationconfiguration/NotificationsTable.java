    ...
    implements NotificationChangeListener
{
    ...
    /**
     * Returns the icon value of the given column if the column supports an
     * icon value, otherwise returns null.
     * @param column the number of the column
     * @return the icon value of the given column if the column supports an
     * icon value, otherwise returns null
     */
    public static ImageIconFuture getColumnIconValue(int column)
    {
        if(column == 1)
        {
            return Resources.getBufferedImage(
    ...
    public void actionAdded(NotificationActionTypeEvent event)
    {
        ...
        switch (event.getActionHandler().getActionType())
        {
            case NotificationAction.ACTION_POPUP_MESSAGE:
                entry.setPopup(isActionEnabled);
                break;
            case NotificationAction.ACTION_COMMAND:
                entry.setProgram(isActionEnabled);

                entry.setProgramFile(((CommandNotificationAction) event
                        .getActionHandler()).getDescriptor());
                break;
            case NotificationAction.ACTION_SOUND:
