    ...
    extends PropertyChangeNotifier
{
    ...
    /**
     * A map of precreated media streams.  These are streams that are created
     * ahead of time, when the format has yet to be decided.  The map is thus
     * from the format of the stream to the stream itself.
     */
    private final Map<MediaFormat, MediaStream> formatStreamMap =
            new HashMap<>();
     ...
     * boolean, boolean)} to {@link CallPeerMediaHandler}s as new instances.
     */
            ...
            MediaType mediaType)
    {
        ...
        logger.debug("streamReferenceCount: " + streamReferenceCount);
     ...
     * @param isPrecreate true if we are precreating the stream, thus the
     * stream shouldn't be stored
     *
     * @return the <tt>MediaStream</tt> that we received as a parameter (for
     * convenience reasons).
     *
     */
    private MediaStream configureStream(
            CallPeerMediaHandler<?> callPeerMediaHandler,
            MediaDevice device,
            MediaFormat format,
            ...
            boolean isPrecreate)
    {
        ...
        if (!isPrecreate)
            configureStreamSrtp(stream, masterStream, call);
         ...
         */
        if (!isPrecreate && stream != callPeerMediaHandler.getStream(mediaType))
    ...
    /**
     * Configure the SRTP controls for a stream
     *
     * @param stream the stream to configure
     * @param masterStream true if this stream is the master stream
     * @param call the call the stream belongs to
     */
    private void configureStreamSrtp(MediaStream stream,
                                     boolean masterStream,
                                     MediaAwareCall<?, ?, ?> call)
    {
            ...
            MediaFormat format)
    {
        ...
        {
        case AUDIO:
            logger.debug("Getting audio stream");
            return audioStream;
        case VIDEO:
    ...
    /**
     * Precreate a stream for a particular format.  Helps to reduce the media
     * cut through by doing the hard work in advance
     *
     * @param callPeerMediaHandler the <tt>CallPeerMediaHandler</tt> that
     * we are creating the stream for
     ...
     * @param masterStream whether the stream to be used as master if secured
     */
    public void precreateStream(CallPeerMediaHandler<?> callPeerMediaHandler,
                                ...
                                boolean masterStream)
    {
        logger.debug("Precreate stream for " + format);

        // Nothing to do if already created the stream
        if (formatStreamMap.containsKey(format))
        {
            logger.debug("Stream already exists " + format);
            return;
        }
        ...
        MediaStream mediaStream = configureStream(callPeerMediaHandler,
                                                  device,
                                                  format,
                                                  target,
                                                  direction,
        ...
        // Store the stream and connector so that they can be retrieved later
        formatStreamMap.put(format, mediaStream);
     ...
     */
    public MediaStream fetchPreCreatedStream(CallPeerMediaHandler<?> mediaHandler,
                                             MediaFormat format)
    {
        logger.debug("Fetch precreated stream for " + format);
        MediaStream mediaStream = formatStreamMap.remove(format);

        // Clear up the remaining streams that are not required
        clearPrefetchedStreams();

        if (mediaStream == null)
        {
            logger.debug("No media stream found for " + format);
            return null;
        }

        // The dynamic payload types have most likely changed, thus update them
        registerDynamicPTsWithStream(mediaHandler, mediaStream);

        // Store that this is the media stream we will be using
        setAudioStream((AudioMediaStream) mediaStream);
        streamReferenceCounts[MediaType.AUDIO.ordinal()]++;

        SrtpControlType srtpControlType = (srtpControls.size() > 0) ?
                                       srtpControls.firstKey().srtpControlType :
                                       SrtpControlType.NULL_IMPL;
        MediaTypeSrtpControl mediaTypeSrtpControl =
                     new MediaTypeSrtpControl(MediaType.AUDIO, srtpControlType);
        ...
        if (srtpControl == null)
        {
             ...
             */
            logger.debug("Using default srtp control");
            srtpControls.put(mediaTypeSrtpControl, mediaStream.getSrtpControl());
            srtpControl = mediaStream.getSrtpControl();
        }

        mediaStream.setSrtpControl(srtpControl);
        configureStreamSrtp(mediaStream, true, mediaHandler.getPeer().getCall());

        return mediaStream;
    }

    /**
     * Clear out the prefetched streams
     */
    public void clearPrefetchedStreams()
    {
        logger.debug("Clear prefetched streams");

        for (Entry<MediaFormat, MediaStream> t : formatStreamMap.entrySet())
        {
            t.getValue().close();
        }
        formatStreamMap.clear();
    }
        ...
        throws OperationFailedException
    {
        ...
        MediaStream stream = getStream(callPeerMediaHandler, mediaType, format);

        if (stream == null)
        {
            logger.info("Creating new media stream with format " + format);
            ...
            /*
             * The default SrtpControlType is NULL. But if a SrtpControl exists
            ...
            MediaStream stream)
    {
        ...
        // Note we must clear any existing overrides before we do so, otherwise
        // we may leave stale overrides lying around.
        stream.clearDynamicRTPPayloadTypeOverrides();
