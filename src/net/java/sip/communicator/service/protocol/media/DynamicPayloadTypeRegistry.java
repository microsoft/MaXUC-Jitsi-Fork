...
public class DynamicPayloadTypeRegistry
{
    ...
    {
        // Create a new format to put in the map so we don't include
        // unnecessary and misleading parameters.
        MediaFormat dynamicFormat = createFormatForMap(format);
        Byte payloadType = getPayloadType(dynamicFormat);
        ...
        if (payloadType == null)
        {
            ...
            Byte preferredPT = getPreferredDynamicPayloadType(dynamicFormat);
            ...
            logger.debug("Adding PT " + payloadType +
                         " to PT mappings for format " + dynamicFormat);
            payloadTypeMappings.put(dynamicFormat, payloadType);
        }
        ...
        throws IllegalArgumentException
    {
        ...
        // unnecessary and misleading parameters.
        MediaFormat dynamicFormat = createFormatForMap(format);
        logger.debug("Adding mapping for PT " + payloadType +
                     " for format: " + dynamicFormat);
        MediaFormat alreadyMappedFmt = findFormat(payloadType);
        if(alreadyMappedFmt != null)
        {
            if(alreadyMappedFmt.matches(dynamicFormat))
            {
                ...
                logger.debug("Already have mapping for format");
            ...
            byte newlyObtainedPT = obtainPayloadTypeNumber(dynamicFormat);

            logger.warn("Remote party is trying to remap payload type "
                        + payloadType + " and reassign it from "
                        + alreadyMappedFmt + " to " + dynamicFormat
                        + ". We'll go along but there might be issues because"
                        + " of this. We'll also expect to receive "
                        + dynamicFormat + " with PT=" + newlyObtainedPT);
        }
        ...
        Byte originalPayloadType = getPayloadType(dynamicFormat);

        if( originalPayloadType != null && originalPayloadType != payloadType)
        {
            logger.debug("Adding to PT overrides (original " +
                         originalPayloadType + " new " + payloadType + ")");
            payloadTypeOverrides.put(originalPayloadType, payloadType);
        }
        else
        {
            //we are just adding a new mapping. nothing out of the ordinary
            logger.debug("Adding to PT mappings");
            payloadTypeMappings.put(dynamicFormat, Byte.valueOf(payloadType));
        }
    }
                                              ...
                                              MediaFormat format)
    {
        ...
        {
            if (mapping.getKey().matches(dynamicFormat))
            {
                Byte pt = mapping.getValue();
                logger.debug("Found PT " + pt + " for format " + dynamicFormat);
                return pt;
            }
        }
    ...
    /**
     * For dynamic payload types, we don't want to be too strict about matching
     * the format against existing formats in our payload type number map. This
     * is because we can have different format parameters in each direction but
     * the codec is the same as it should have the same number.
     *
     * For example, don't create a new payload type for these 2 formats:
     * <ul>
     * <li>H264/90000 fmtp:profile-level-id=42E00B;packetization-mode=0</li>
     * <li>H264/90000 fmtp:profile-level-id=42E01F;packetization-mode=0</li>
     * </ul>
     *
     * @param format The format to use to create our format for the map
     * @return A new format to be used for out dynamic payload map
     */
    private static MediaFormat createFormatForMap(MediaFormat format)
    {
        // Create a temporary format here with just the encoding and clock rate.
        return ProtocolMediaActivator.getMediaService()
            .getFormatFactory().createMediaFormat(format.getEncoding(),
                format.getClockRate());
    }
}
