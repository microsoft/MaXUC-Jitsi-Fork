               ...
               PropertyChangeListener
{
    ...
    /**
     * Our video receiving policy.
     */
    private boolean remoteVideoAllowed = true;
    ...
    public void peerStateChanged(CallPeerChangeEvent evt)
    {
        ...
        logger.debug("peerStateChanged to " + newState);
    ...
    /**
     * Modifies the local media setup of all peers in the call to reflect the
     * requested setting for the receiving of the remote video and then passes
     * the setting to the participating <tt>MediaAwareCallPeer</tt> instances.
     *
     * @param allowed whether remote video id allowed
     * @param useCase the use case of the video (i.e video call or desktop
     * streaming/sharing session)
     *
     * @throws OperationFailedException if video initialization fails.
     */
    public void setRemoteVideoAllowed(boolean allowed, MediaUseCase useCase)
        throws OperationFailedException
    {
        remoteVideoAllowed = allowed;
        mediaUseCase = useCase;
        ...
        while (peers.hasNext())
        {
            peers.next().setRemoteVideoAllowed(allowed);
        }
    }
    ...
    {
        return mediaUseCase.equals(MediaUseCase.CALL) && localVideoAllowed;
    }

    /**
     * Determines whether the receiving of remote video in this <tt>Call</tt>
     * is currently allowed. The setting does not reflect the availability of
     * actual video capture devices, it just expresses the local policy (or
     * desire of the user) to receive the remote video in the case that the
     * system is actually able to do so.
     *
     * @return <tt>true</tt> if the streaming of local video for this
     * <tt>Call</tt> is allowed; otherwise, <tt>false</tt>
    ...
    {
        return (mediaUseCase.equals(MediaUseCase.CALL) && remoteVideoAllowed);
    }
