...
public class AccountManager
{
        ...
        if ((factoryRefs != null) && (factoryRefs.length > 0))
        {
            ...
            for (ServiceReference<?> factoryRef : factoryRefs)
            {
                    ...
                    storedAccounts.iterator(); storedAccountIter.hasNext();)
                {
                    String storedAccount = storedAccountIter.next();
                    List<String> storedAccountProperties =
                        configService.user().getPropertyNamesByPrefix(storedAccount,
                            true);
    ...
    /**
     * Checks whether this account manager has processed the given protocol
     * provider factory
     *
     * @param protocolName the name of the protocol to check
     * @return whether the protocol provider factory has been processed
     */
    public boolean isProtocolFactoryProcessed(String protocolName)
    {
        Boolean isProcessed = false;
        // Check the protocol provider factory has registered and is not
        // waiting to be processed.
        for (ProtocolProviderFactory factory : factories)
        {
            if (factory.getProtocolName().equals(protocolName) &&
                !loadStoredAccountsQueue.contains(factory))
            {
                isProcessed = true;
            }
        }
