    ...
    extends OperationSetPresence
{
    ...
    /**
     * Returns whether it is possible to unsubscribe from a particular contact
     * or not.
     *
     * @param contact the contact that we want to unsubscribe from
     * @return true if unsubscribe is supported
     */
    boolean unsubscribeSupported(Contact contact);
     ...
     */
    void removeServerStoredGroupChangeListener(
            ServerStoredGroupListener listener);
    ...
    /**
     * Called when the MclStorageManager has finished creating unresolved
     * contacts (from contactlist.xml) and thus it is time to start getting
     * contacts from the server and resolve the ones that have been loaded.
     */
    void finishedLoadingUnresolvedContacts();
    ...
    /**
     * The contacts for the server stored contact list may not have address
     * fields that are appropriate to show to the user, e.g. if the address
     * field is a server generated UID.  This method returns false if it is
     * not suitable for display.
     *
     * @return true if the Contact List stored by this service contains contacts
     *         whose address is suitable for display
     */
    boolean isAddressDisplayable();
}
