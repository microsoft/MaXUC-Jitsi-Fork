...
jmethodID
AddrBookContactQuery_getPtrQueryCompleteMethodID(JNIEnv *jniEnv, jobject callback)
{
    jclass callbackClass;
    jmethodID callbackMethodID = 0;
    ...
    if (callback)
    {
        ...
        if (callbackClass)
        {
            ...
            {
                Exception_throwNew(
                    jniEnv, "java/lang/IllegalArgumentException", "queryCompleted");
            }
        }
    }
    else
    {
        Exception_throwNew(
            jniEnv, "java/lang/NullPointerException", "queryComplete");
    }
    return callbackMethodID;
}
