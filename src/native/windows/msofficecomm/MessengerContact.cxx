...
STDMETHODIMP MessengerContact::get_PhoneNumber(MPHONE_TYPE PhoneType, BSTR *bstrNumber)
{
    ...
    Log::d(_T("MessengerContact::getPhoneNumber Contact %s\n"), _signinName);
    ...
    if (bstrNumber)
    {
        if (_jobject)
        {
            ...
            if (vm && !(vm->AttachCurrentThreadAsDaemon((void **) &env, NULL)))
            {
                ...
                if (env->ExceptionCheck())
                {
                    Log::d(_T("\nMessengerContact::get_PhoneNumber exception"));
                    env->ExceptionClear();
                    *bstrNumber = NULL;
                    hr = E_FAIL;
                }
    ...
    Log::d(_T("MessengerContact::getPhoneNumber Got %S\n"), *bstrNumber);
    return hr;
}
...
MessengerContact::get_PresenceProperties(VARIANT *pvPresenceProperties)
{
    ...
    Log::d(_T("MessengerContact::get_PresenceProperties Contact %s\n"),
           _signinName);
    if (FAILED(hr))
    {
        return hr;
    ...
    Log::d(_T("MessengerContact::get_PresenceProperties Return %lx\n"),
           hr);
...
STDMETHODIMP MessengerContact::get_Status(MISTATUS *pMstate)
{
    ...
    Log::d(_T("MessengerContact::get_Status Contact %s\n"), _signinName);
    ...
    Log::d(_T("MessengerContact::get_Status Status is %d\n"), *pMstate);
