...
Messenger::createMessengerContact(BSTR signinName, REFIID iid, PVOID *obj)
{
    ...
    Log::d(_T("Messenger::createMessengerContact Creating %s\n"), signinName);
    ...
    Log::d(_T("Messenger::createMessengerContact Returning %lx\n"), hr);
...
Messenger::GetContact(BSTR bstrSigninName, BSTR bstrServiceId, IDispatch **ppMContact)
{
    ...
    Log::d(_T("Messenger::GetContact Looking for %s\n"), bstrSigninName);
    ...
    Log::d(_T("Messenger::GetContact Found %p\n"), *ppMContact);
...
HRESULT Messenger::getMessengerContact(BSTR signinName, REFIID iid, PVOID *obj)
{
    *obj = NULL;
    ...
    Log::d(_T("Messenger::getMessengerContact Looking for %s\n"), signinName);
    ...
    Log::d(_T("Messenger::getMessengerContact Found %p\n"), *obj);
...
void CALLBACK Messenger::onContactStatusChange(ULONG_PTR dwParam)
{
    ...
    if (run)
    {
        ...
        if (thiz && thiz->_dMessengerEventsConnectionPoint)
        {
            ...
            Log::d(_T("Messenger::onContactStatusChange Contact %s status change\n"), event->_signinName);
        ...
        if (clazz)
        {
            ...
            if (ctorMethodID)
            {
                ...
                if (startConversationMethodID)
                {
                    ...
                    {
                        Log::d(_T("Cannot find global ref\n"));
                        hr = E_OUTOFMEMORY;
                    }
                }
                else
                {
                    Log::d(_T("Cannot start conversation\n"));
                    hr = E_FAIL;
                }
            }
            else
            {
                Log::d(_T("Cannot find init method\n"));
                hr = E_FAIL;
            }
        }
        else
        {
            Log::d(_T("Cannot find class\n"));
            hr = E_FAIL;
        }
    }
    else
     ...
     VARIANT *pvWndHnd)
{
    ...
    Log::d(_T("Messenger::StartConversation Starting\n"));
    ...
    if (vm && !(vm->AttachCurrentThreadAsDaemon((void **) &env, NULL)))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                Log::d(_T("Messenger::StartConversation Failed to get "
                          "conversation data (hr %d)\n"), hr);
            }
        }
        else
        {
            Log::d(_T("Messenger::StartConversation Failed to get "
                      "participant array (hr %d)\n"), hr);
        }
    ...
    {
        Log::d(_T("Messenger::StartConversation Failed to attatch JNI"));
        hr = E_UNEXPECTED;
    }
    return hr;
}
...
HRESULT Messenger::toString(JNIEnv *env, VARIANT &v, jstring *string)
{
    ...
    {
        // Empty data is also a success (but a NULL string).  This may occur if
        // a contact doesn't have full information (e.g. no e-mail address) but
        // we may still be able to complete setup from the conversation data.
        hr = S_OK;
    }
    else
    {
        Log::d(_T("Messenger::toString Unexpected varient type: %d\n"),
               v.vt);
        hr = E_INVALIDARG;
    }
...
Messenger::toStringArray(JNIEnv *env, VARIANT &v, jobjectArray *stringArray)
{
    ...
    if (VT_ARRAY == (v.vt & VT_ARRAY))
    {
        ...
        {
            Log::d(_T("Messenger::toStringArray Unexpected array type: %d "
                      "%d\n"),
                   sa->cDims,
                   sa->fFeatures);
            hr = E_INVALIDARG;
    ...
    {
        Log::d(_T("Messenger::toStringArray Unexpected varient type: %d\n"),
               v.vt);
        hr = E_INVALIDARG;
    }
    return hr;
